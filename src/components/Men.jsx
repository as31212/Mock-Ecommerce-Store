...
const Men = ({data,dropdown,changeDropdown,changeCurrItem})=>{


    const items = dropdown === 'low-high' ? [...data].sort((a,b)=> a.sale_price - b.sale_price).filter(el=> el.clothing_type === 'Men').map((el) => {
        return (
    ...
    }) : [...data].sort((a,b)=> b.sale_price - a.sale_price).filter(el=> el.clothing_type === 'Men').map((el) => {
        return (
          <div onClick={()=>{changeCurrItem(el.id)}} className={`item ${el.clothing_type}`} id={el.id} key={el.id}>
            <Link to= '/Purchase'>
            <img src={el.img} alt={`${el.item_name} picture`} />
